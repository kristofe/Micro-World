...
@torch.no_grad()
def log_validation(vae, text_encoder, tokenizer, transformer3d, network, text, config, args, accelerator, weight_dtype, global_step):
    try:
        ...
        pipeline = WanPipeline(
            ...
            tokenizer=tokenizer,
            transformer=transformer3d_val, #accelerator.unwrap_model(transformer3d),
            scheduler=scheduler,
        )
        ...
        with accelerator.autocast():
            ...
            save_videos_grid(sample, os.path.join(args.output_dir, f"sample/sample-{global_step}.mp4"), fps=15)

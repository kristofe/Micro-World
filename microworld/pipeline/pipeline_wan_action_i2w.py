...
from .pipeline_wan_action_t2w import parse_config, draw_keys_on_frame, overlay_icon
from ..models import (AutoencoderKLWan, AutoTokenizer,
                              WanT5EncoderModel, WanTransformer3DModel, CLIPModel)
...
class WanActionI2WPipeline(DiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        self.register_modules(
            tokenizer=tokenizer, text_encoder=text_encoder, vae=vae, transformer=transformer, scheduler=scheduler, clip_image_encoder=clip_image_encoder
        )
    ...
    def __call__(
        ...
        if video is not None:
            bs, _, video_length, height, width = video.shape
            init_video = self.image_processor.preprocess(rearrange(video, "b c f h w -> (b f) c h w"), height=height, width=width)
            init_video = init_video.to(dtype=torch.float32)
            init_video = rearrange(init_video, "(b f) c h w -> b c f h w", f=video_length)

            mask_video_length = mask_video.shape[2]
            mask_condition = self.mask_processor.preprocess(rearrange(mask_video, "b c f h w -> (b f) c h w"), height=height, width=width)
            mask_condition = mask_condition.to(dtype=torch.float32)
            mask_condition = rearrange(mask_condition, "(b f) c h w -> b c f h w", f=mask_video_length)

...
def parse_action_list(action_list):
    """
    Generate key data and mouse data from action list.
    - action_list: list of actions
    - return: key_data and mouse_data
    """
    key_data = [[0, 0, 0, 0, 0, 0, 0]]
    mouse_data = [[0,0]]
    space_frames = set()
    if action_list[-1]:
        space_frames = set(map(int, action_list[-1].split()))
    for i in range(len(action_list) - 1):
        end_frame, action = action_list[i]
        w, s, a, d, shift, ctrl, _, mouse_y, mouse_x = map(float, action.split())
        # Count the start frame of the previous segment
        start_frame = 1 if i == 0 else action_list[i - 1][0] + 1

        # Fill the frame range data
        for frame in range(start_frame, int(end_frame) + 1):
            # Key status
            cur_key_data = [int(w), int(s), int(a), int(d), int(frame in space_frames), int(shift), int(ctrl)]
            key_data.append(cur_key_data)
            # Mouse position
            cur_mouse_data = [float(mouse_y), float(mouse_x)]
            mouse_data.append(cur_mouse_data)
    return torch.from_numpy(np.stack(key_data)).float(), torch.from_numpy(np.stack(mouse_data)).float()